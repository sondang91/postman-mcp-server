 ...
 */
export function parseControllerEndpoints(controllerCode, options) {
    const endpoints = [];
    // Extract controller name
    const controllerNameMatch = controllerCode.match(/public\s+class\s+(\w+Controller)\s*:/);
    ...
    const versionMatch = controllerCode.match(/\[ApiVersion\(["']([^"']+)["']\)\]/);
    ...
    for (const httpMethodMatch of httpMethodMatches) {
        ...
        const methodSignatureMatch = afterHttpMethod.match(/public\s+(?:async\s+)?Task<[^>]+>\s+(\w+)\s*\(/m);
        ...
        if (methodSignatureEnd === -1)
            continue;
        const methodSignature = controllerCode.substring(methodStartIndex, methodSignatureEnd + 1);
        ...
        const headers = Array.from(headersMap.values());
        endpoints.push({
            name: requestName,
 ...
 */
function extractParameters(methodSignature) {
    const parameters = [];
    ...
    const paramRegex = /(?:\[From(Query|Route|Body)(?:\(Name\s*=\s*["'](\w+)["']\))?\])?\s*(\w+(?:<[^>]+>)?\??)\s+(\w+)(?:\s*=\s*[^,)]+)?/g;
    let match;
    while ((match = paramRegex.exec(paramsSection)) !== null) {
        ...
        if (type.includes("PagedRequest")) {
            parameters.push({ name: "pageNumber", type: "int", location: "query", required: false }, { name: "pageSize", type: "int", location: "query", required: false }, { name: "searchTerm", type: "string", location: "query", required: false });
            continue;
        }
 ...
 */
export function generateRequestBodyExample(parameters) {
